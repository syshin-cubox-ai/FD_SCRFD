...
def check_img_size(imgsz, s=32, floor=0):
    ...
    if new_size != imgsz:
        print(
            f"WARNING: img_size {imgsz} must be multiple of max stride {s}, updating to {new_size}"
        )
    return new_size


def resize_preserving_aspect_ratio(
    img: np.ndarray, img_size: int, scale_ratio=1.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--skip_simplify", action="store_true", help="skip onnx-simplifier"
    )
    ...
    output_path = os.path.join(
        output_dir, os.path.basename(args.config).replace(".py", ".onnx")
    )
